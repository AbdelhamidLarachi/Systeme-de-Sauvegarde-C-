...
class BackupDirectory
{
	...
	public static void CopyAll(DirectoryInfo source, DirectoryInfo target)
	{
		...
		{
			if (filesNBcount <= totalFolderFiles)
			{
				filesNBcount++;
				...
				// Write string to file in json format
				Thread.Sleep(1000);  // a second sleep for each file is copied (just to look cool :D)
			}
		}
	...
	public static void Differential(DirectoryInfo source, DirectoryInfo destination)
	{
		...
		foreach (FileInfo file in files)
		{
			foreach (FileInfo fileD in destFiles)
			{
				...
				{
                    filesNBcount++;
                    string newDir = destination.Parent + "/onlyModified";
                    Directory.CreateDirectory(newDir);  // create a new directory where to put modified files
                    file.CopyTo(Path.Combine(destination.FullName, newDir+"/"+file.Name), true); drawTextProgressBar(totalFolderFiles); // Backup progress Bar
				...
				else if (!File.Exists(Path.Combine(destination.FullName, file.Name)))
				{
                    ...
                    file.CopyTo(Path.Combine(destination.FullName, newDir+"/"+file.Name), true);
					drawTextProgressBar(totalFolderFiles); // Backup progress Bar
					RealTimeJson(file, totalFolderFiles, destination);
